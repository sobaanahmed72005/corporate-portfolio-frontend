...
export async function OfficesSection() {
  ...
  return (
    <section className="bg-ink-950 py-16 text-ink-50 sm:py-24">
      <Container>
        ...
        <div className="mt-10 grid grid-cols-1 gap-6 sm:grid-cols-2">
          {offices.map((office) => (
            <div
              key={office.slug}
              className="flex flex-col gap-4 rounded-2xl border border-ink-50/15 bg-ink-50/5 p-6"
            >
              ...
              <ul className="space-y-2 text-sm text-ink-200">
                <li className="flex items-start gap-2">
                  ...
                  <a href={`tel:${office.phone}`} className="hover:text-ink-50">
                    {office.phone}
                  </a>
                </li>
                <li className="flex items-start gap-2">
                  <Mail className="mt-0.5 h-4 w-4 shrink-0" aria-hidden />
                  <a href={`mailto:${office.email}`} className="hover:text-ink-50">
                    {office.email}
                  </a>
                </li>
                <li className="flex items-start gap-2">
