      ...
      <Container>
        ...
        <div className="mt-10 grid grid-cols-1 gap-6 lg:grid-cols-[1.3fr_1fr]">
          <Link
            ...
            className="group flex flex-col justify-end rounded-3xl border border-cardText-950/10 bg-gradient-to-br from-card-800 to-card-950 p-8 transition-colors hover:border-cardText-950/25 sm:p-10"
          >
            <p className="font-display text-xs font-semibold uppercase tracking-wide text-brand-300">
              {featured.category}
            </p>
            <h3 className="mt-3 font-display text-2xl font-extrabold text-cardText-950 sm:text-3xl">
              {featured.title}
            </h3>
            <p className="mt-3 max-w-lg text-sm leading-relaxed text-cardText-600">{featured.excerpt}</p>
            <span className="mt-5 inline-flex items-center gap-1.5 text-sm font-semibold text-cardText-950">
              Read the article
          ...
          <div className="flex flex-col divide-y divide-cardText-950/10 rounded-3xl border border-cardText-950/10">
            {rest.map((post) => (
              <Link
                key={post.slug}
                href={`/blog/${post.slug}`}
                className="flex flex-col gap-1 p-5 transition-colors hover:bg-cardText-950/5"
              >
                ...
                <p className="text-xs text-cardText-9500">{post.date}</p>
              </Link>
            ))}
          </div>
        </div>
