      ...
      <Container>
        ...
        <div className="mt-10 grid grid-cols-1 gap-6 lg:grid-cols-[1.3fr_1fr]">
          <Link
            ...
            className="group flex flex-col justify-end rounded-3xl border border-ink-50/10 bg-gradient-to-br from-ink-800 to-ink-950 p-8 transition-colors hover:border-ink-50/25 sm:p-10"
          >
            <p className="font-display text-xs font-semibold uppercase tracking-wide text-brand-300">
              {featured.category}
            </p>
            <h3 className="mt-3 font-display text-2xl font-extrabold text-ink-50 sm:text-3xl">
              {featured.title}
            </h3>
            <p className="mt-3 max-w-lg text-sm leading-relaxed text-ink-400">{featured.excerpt}</p>
            <span className="mt-5 inline-flex items-center gap-1.5 text-sm font-semibold text-ink-50">
              Read the article
              <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" aria-hidden />
            </span>
          </Link>

          <div className="flex flex-col divide-y divide-ink-50/10 rounded-3xl border border-ink-50/10">
            {rest.map((post) => (
              <Link
                key={post.slug}
                href={`/blog/${post.slug}`}
                className="flex flex-col gap-1 p-5 transition-colors hover:bg-ink-50/5"
              >
                <p className="font-display text-[11px] font-semibold uppercase tracking-wide text-brand-300">
                  {post.category}
                </p>
                <h4 className="line-clamp-2 text-sm font-semibold text-ink-50">{post.title}</h4>
